...
- 需要协作或审查的改动使用短生命周期分支并合入 `main`。任何 push/PR/发包等对外发布动作仍受 [`rules/system.md`](../../rules/system.md) 的确认要求约束。
...
- 根 `.github/CODEOWNERS` 当前将全仓指派给 `@Go1c`；涉及 Tick 顺序、ECS 状态、CrossWorldTxn、Replication、持久化或 Hot Reload 的改动必须由对应所有者审查。
- 合并前必须通过 Repository Policy、适用的 Replay/Differential/Soak 证据、收口门槛与 reviewer 闭环；不在 Runtime 中接受具体玩法或 Host/平台类型。
